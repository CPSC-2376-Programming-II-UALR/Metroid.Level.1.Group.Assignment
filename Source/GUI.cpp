...
void GUI::loadMedia(){
	...
	fstream fin("./Assets/Config/spritePaths.txt");
	string path;
	while (!fin.eof())
	{
		fin >> path;
	...
	//Open the font
	normalFont = TTF_OpenFont("./Assets/Fonts/impact.ttf", (int)(screenWidth*.06));
	if (normalFont == NULL) {
		printf("Failed to load lazy font! SDL_ttf Error: %s\n", TTF_GetError());
	}
	//Open the font
	smallFont = TTF_OpenFont("./Assets/Fonts/impact.ttf", (int)(screenWidth*.02));
	if (smallFont == NULL) {
		printf("Failed to load lazy font! SDL_ttf Error: %s\n", TTF_GetError());
	}
}
...
void GUI::displayGameState(Engine* engine)
{
	...
	for (const auto& object : engine->getObjects())
	{
		...
		case Object::Type::Pedestal:
			...
			break;
		case Object::Type::player:
			playerSheetTexture->render(renderer, object->getPosition(), &playerClips[((AnimatedObject*)object.get())->getCurrentSpriteID()]);
			break;
		case Object::Type::skree:
			skreeSheetTexture->render(renderer, object->getPosition(), &skreeClips[((AnimatedObject*)object.get())->getCurrentSpriteID()]);
			break;
		default:
			break;
		}
	...
	SDL_Rect renderQuad = { screenWidth-width-5, height, width, height };
